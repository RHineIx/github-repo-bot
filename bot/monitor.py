...
{body}</blockquote>
    ...
    async def _check_stars_updates(self, repo_data: Dict):  
        ...
        starred_repos = await self.github_api.get_authenticated_user_starred_repos(page=1, per_page=10)  
          
        if starred_repos:  
            current_starred_ids = {str(repo.get('id')) for repo in starred_repos}  
            new_starred_ids = current_starred_ids - last_starred_repos  
              
            if new_starred_ids and last_starred_repos:  # Only notify if we had previous data  
                # Find the new starred repositories  
                new_repos = [repo for repo in starred_repos if str(repo.get('id')) in new_starred_ids]  
                  
                for new_repo in new_repos:  
                    await self._send_starred_repo_notification(repo_data, new_repo)  
              
            # Update stored starred repos  
            await self.tracker.update_last_starred_repos(github_username, current_starred_ids)
    ...
    async def _send_starred_repo_notification(self, repo_data: Dict, starred_repo: Dict):  
        ...
        if repo_data_full:  
            ...
            preview = RepoFormatter.format_repository_preview(repo_data_full, languages, latest_release)  
            ...
            for user_id in subscribers:
                try:  
                    await self.bot.send_message(user_id, notification_text)  
                    logger.info(f"Sent starred repo notification to user {user_id} for {owner}/{repo_name}")  
                except Exception as e:  
                    logger.error(f"Failed to send starred repo notification to user {user_id}: {e}")